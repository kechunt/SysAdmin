...
menu_principal() {
  while true; do
    ...
    echo '  [4] Instalar y asegurar SSH       (Práctica 4)  ← última vez en consola'
    echo '  [5] Diagnóstico DHCP / DNS'
    echo '  [6] Probar resolución DNS (cliente)'
    echo '  [7] Probar / abrir SSH (cliente)'
    echo '  [8] Salir'
    read -r -p 'Opción: ' op
    case $op in
      1) menu_diagnostico ;;
      2) dhcp_configurar ;;
      3) dns_configurar ;;
      4) ssh_instalar; ssh_diagnostico ;;
      5)
