...
ftp_asegurar_nologin_en_shells() {
  # pam_shells.so (PAM de vsftpd) rechaza el login si el shell no está en /etc/shells.
  local sh
  for sh in /usr/sbin/nologin /sbin/nologin; do
    [[ -e $sh ]] || continue
...
EOF
  ...
  # userlist_deny=NO = lista blanca. Sin ftp/anonymous el anónimo da 530 Permission denied.
  ftp_agregar_userlist ftp
  ftp_agregar_userlist anonymous
}
...
ftp_crear_usuario() {
  ...
  [[ -n $pass ]] || die "No se recibió contraseña para $user."
  ftp_asegurar_nologin_en_shells
  printf '%s:%s\n' "$user" "$pass" | chpasswd
  usermod -s /usr/sbin/nologin -G "ftpusers,${grupo}" "$user"
