...
ask_puerto_http() {
  ...
  while true; do
    ...
    if http_puerto_en_uso "$value"; then
      echo "El puerto $value ya está ocupado por otro servicio. Elija otro." >&2
      continue
    fi
    printf '%s' "$value"
    return
...
http_apt_update() {
  apt-get update -qq || die 'apt-get update falló. No se pueden consultar versiones.'
}
...
ask_version_lista() {
  ...
  echo
  echo "Versiones detectadas para ${titulo} (consulta en vivo):"
  for i in "${!lineas[@]}"; do
    printf '  [%d] %s\n' $((i + 1)) "${lineas[$i]}"
  done
  while true; do
    read -r -p 'Seleccione versión: ' sel
...
http_tomcat_versiones() {
  ...
  local html rel
  html=$(curl -fsSL --max-time 15 'https://downloads.apache.org/tomcat/tomcat-10/' 2>/dev/null || true)
...
http_instalar_tomcat_tarball() {
  ...
  local url="https://downloads.apache.org/tomcat/tomcat-10/${tag}/bin/apache-tomcat-${ver}.tar.gz"
  local dest=/opt/tomcat
  mkdir -p /opt
  curl -fL --max-time 120 "$url" -o /tmp/tomcat.tgz || die "No se pudo descargar $url"
  rm -rf "$dest"
  mkdir -p "$dest"
...
menu_http() {
  while true; do
    ...
    echo '=================================================='
    echo '  [1] Instalar Apache2   (versiones apt-cache madison)'
