...
ssl_tomcat() {
  ...
  [[ -n $conf ]] || die 'Tomcat no está instalado (no hay server.xml). Instale antes por WEB o FTP.'
  backup_archivo "$conf"
...
" "$conf"
  ...
  ok "Tomcat HTTPS en ${https} (PKCS12)"
}

ssl_vsftpd() {
  ssl_generar_cert
  [[ -f /etc/vsftpd.conf ]] || die 'vsftpd no está instalado (Práctica 5).'
...
EOF
  ...
  ok "vsftpd FTPS (explícito, control+datos) con cert ${SSL_CN}"
}
...
ssl_menu_servicios() {
  echo
  echo '¿Qué servicio desea cifrar?'
