...
ftp_repo_preparar() {
  ...
  echo
  info 'Los clientes anónimos ven: ftp://IP/http/Linux/<Servicio>/archivo'
}

ftp_ask_conexion() {
  FTP_HOST=$(ask_ip 'IP del servidor FTP (Práctica 5) [10.10.10.10]: ' '10.10.10.10')
  read -r -p 'Usuario FTP [anonymous]: ' FTP_USER
  FTP_USER=${FTP_USER:-anonymous}
  [[ $FTP_USER != *['!@#$%^&*;<>|`']* ]] || die 'Usuario FTP inválido.'
  if [[ $FTP_USER == anonymous ]]; then
    FTP_PASS='anonymous'
  else
    read -r -s -p 'Contraseña FTP: ' FTP_PASS
    echo
    [[ -n $FTP_PASS ]] || die 'Contraseña vacía.'
  fi
}
...
ask_indice() {
  ...
  local -a items=("$@")
  local i sel n
  [[ ${#items[@]} -ge 1 ]] || die "No hay entradas para $titulo (¿repositorio vacío o ruta incorrecta?)."
  echo
  echo "$titulo"
  for i in "${!items[@]}"; do
    printf '  [%d] %s\n' $((i + 1)) "${items[$i]}"
  ...
  while true; do
    ...
    [[ $sel =~ ^[1-9][0-9]*$ ]] || { echo 'Número inválido.' >&2; continue; }
    n=$((sel - 1))
...
ftp_verificar_hash() {
  ...
  if [[ -f ${bin}.sha256 ]]; then
    hfile=${bin}.sha256
    (cd "$dir" && sha256sum -c "$(basename "$hfile")") || die "Integridad SHA256 FALLÓ para $base"
  ...
  if [[ -f ${bin}.md5 ]]; then
    ...
    [[ $localh == "$remoteh" ]] || die "Integridad MD5 FALLÓ para $base"
...
ftp_navegar_descargar() {
  local os servicio archivo url_base
  ftp_ask_conexion
  os=$(ask_indice 'Sistema en el repositorio /http/' 'Linux' 'Windows')
  ...
  ftp_curl_get "${url_base}${servicio}/${archivo}.sha256" "${FTP_DL}/${archivo}.sha256" 2>/dev/null \
    || ftp_curl_get "${url_base}${servicio}/${archivo}.md5" "${FTP_DL}/${archivo}.md5"
  ftp_verificar_hash "${FTP_DL}/${archivo}"
  printf '%s' "${FTP_DL}/${archivo}"
}
