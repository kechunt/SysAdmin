...
function Initialize-FtpRepoSeed {
    ...
    Write-Host 'Si Chocolatey está instalado, se copiarán nupkgs de apache-httpd/nginx si existen en la caché.'
...
function Save-FtpFile {
    ...
    if (Get-Command curl.exe -ErrorAction SilentlyContinue) {
        ...
        if ($LASTEXITCODE -ne 0) { throw "curl no descargó $Url" }
        return
    }
    $cred = New-Object Net.NetworkCredential($script:FtpUser, $script:FtpPass)
...
function Test-FtpFileHash {
    ...
    if (Test-Path $sha) {
        ...
        if ($local -ne $remote) { throw "Integridad SHA256 FALLÓ para $name" }
    ...
    if (Test-Path $md5) {
        ...
        if ($local -ne $remote) { throw "Integridad MD5 FALLÓ para $name" }
...
function Invoke-FtpNavigateDownload {
    Read-FtpConnection
    $os = Read-Choice -Titulo 'Sistema en /http/' -Items @('Linux', 'Windows')
...
function Install-FtpBinary {
    ...
    switch ($ext) {
        '.msi' { Start-Process msiexec.exe -ArgumentList "/i `"$Bin`" /qn /norestart" -Wait }
        '.exe' { Start-Process $Bin -ArgumentList '/S','/quiet','/qn' -Wait }
        '.zip' {
            $out = 'C:\p7-unpack'
